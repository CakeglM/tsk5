...
private: System::Void tabControl1_SelectedIndexChanged(System::Object^ sender, System::EventArgs^ e) {
    ...
    if ((textBox1->Text != "") && (textBox2->Text != "") && (textBox3->Text != "") && (textBox4->Text != "")) {
        ...
        else
        {
            ...
            i = 1; x = xn; ymax = -
                ...
                (x <= xk)
            {
                if (x <= 0) { y =pow(x+2,3)-1 ; }
                else
                    if (x > 0 && x <= 5) y =sin(pow(x,2)+pow(x,0.25)) ;

                    else
                        if (x > a) { y =pow(log(x),2)+x; }
    ...
    {
        MessageBox::Show("Çàïîëíèòå,äàííûå", "Îøèáêà ââîäà äàííûõ", MessageBoxButtons::OK, MessageBoxIcon::Exclamation);
    }
}
...
private: System::Void öâåòÔîíàÃğàôèêàToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
    Series^ plot = chart->Series[0];
    if (colorDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
        chart->BackColor = colorDialog1->Color;
    }
}
private: System::Void ñîõğàíèòüÄàííûåToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    if (saveFileDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK)
    {
        ...
        for (int i = 0; i < dataGridView1->RowCount; i++) pwriter->WriteLine(dataGridView1->Rows[i]->Cells[0]->Value->ToString(
